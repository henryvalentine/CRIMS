...
$(function () {
        ...
        success: function (response) {
            ...
            if (/Approval/.test(window.location.href)) {
                ...
                if (approvalTable !== undefined && approvalTable !== null) {
                    ...
                    approvedTableOptions.columnHeaders = ['ProjectPrimaryCode', 'Surname', 'Firstname', 'EnrollmentOfficer', 'EnrollmentDateStr'];
    ...
    if (/BaseData/.test(window.location.href)) {
        ...
        if (enrolleeTable !== undefined && enrolleeTable !== null)
        {
            ...
            tableOptions.columnHeaders = [
                //'ProjectPrimaryCode', 'Surname', 'Firstname', 'BiometricStatus', 'EnrollmentOfficer', 'EnrollmentDateStr'
                'ProjectPrimaryCode', 'Surname', 'Firstname', 'EnrollmentOfficer', 'EnrollmentDateStr'
            ];
    ...
    if (/Rejections/.test(window.location.href)) {
        ...
        if (rejectionTable !== undefined && rejectionTable !== null) {
            ...
            rejectionTableOptions.columnHeaders = ['ProjectPrimaryCode', 'Surname', 'Firstname', 'EnrollmentOfficer', 'EnrollmentDateStr'];
...
function approvalHistory(itemId) {
    ...
    $.ajax({
        ...
        success: function (response) {
            window.hideUiBusy();
            if (!response || response.EnrollmentId === undefined || response.EnrollmentId === null || response.EnrollmentId.length < 1) {
...
function processBaseData() {
    window.baseDataScope.Email = $("#Email").val().trim() !== undefined && $("#Email").val().trim() !== null && $("#Email").val().trim().length > 0? $("#Email").val().trim().toUpperCase() : '';
    window.baseDataScope.ProjectPrimaryCode = $("#baseDataTemplate #ProjectPrimaryCode").val().trim() !== undefined && $("#baseDataTemplate #ProjectPrimaryCode").val().trim() !== null && $("#baseDataTemplate #ProjectPrimaryCode").val().trim().length > 0? $("#baseDataTemplate #ProjectPrimaryCode").val().toUpperCase() : '';
    var confirmProjectPrimaryCode = $("#baseDataTemplate #confirmProjectPrimaryCode").val().trim() !== undefined && $("#baseDataTemplate #confirmProjectPrimaryCode").val().trim() !== null && $("#baseDataTemplate #confirmProjectPrimaryCode").val().trim().length > 0 ? $("#baseDataTemplate #confirmProjectPrimaryCode").val().toUpperCase() : '';
    window.baseDataScope.MobileNumber = $("#MobileNumber").val().trim() !== undefined && $("#MobileNumber").val().trim() !== null && $("#MobileNumber").val().trim().length > 0? $("#MobileNumber").val().trim() : '';
    window.baseDataScope.Surname = $("#Surname").val().trim() !== undefined && $("#Surname").val().trim() !== null && $("#Surname").val().trim().length > 0? $("#Surname").val().trim().toUpperCase() : '';
    window.baseDataScope.Gender = $("#Gender").val();
    window.baseDataScope.Title = $("#Title").val();
    window.baseDataScope.CuntryCode = $("#CuntryCode").val();
    window.baseDataScope.MiddleName = $("#MiddleName").val() !== undefined && $("#MiddleName").val() !== null && $("#MiddleName").val().length > 0? $("#MiddleName").val().toUpperCase() : '';
    window.baseDataScope.Firstname = $("#Firstname").val().trim() !== undefined && $("#Firstname").val().trim() !== null && $("#Firstname").val().trim().length > 0? $("#Firstname").val().trim().toUpperCase() : '';
    window.baseDataScope.ValidIdNumber = $("#ValidIdNumber").val().trim() !== undefined && $("#ValidIdNumber").val().trim() !== null && $("#ValidIdNumber").val().trim().length > 0? $("#ValidIdNumber").val().trim().toUpperCase() : '';
    ...
    if (window.baseDataScope.Surname.length < 2) {
    ...
    if (window.baseDataScope.Firstname.length < 2) {
...
function clearBiometricControls()
{
    $('#photo').attr('src', '');
    $('#lLittle').attr('src', '');
    $('#lRing').attr('src', '');
...
function InitialiseBaseDataUi(itemId, tblList, customDataView) {
    ...
    $.ajax({
        ...
        success: function (data) {
            window.hideUiBusy();
            if (!data || data.EnrollmentId === undefined || data.EnrollmentId === null || data.EnrollmentId.length < 1 || data.CustomGroupViewModels.length < 1)
            {
                    ...
                    $.each(customFields, function (j, f) {
                        ...
                        if (f.CustomFieldType.FieldTypeName === 'List' && f.CustomList !== undefined && f.CustomList !== null) {
                            ...
                            if (qq.HasChildren === true) {
                                tabpanel += '<select data-edit-id="" style="text-transform: capitalize" data-required="' + req + '" data-has-child="true" data-pList="' + qq.ParentListId + '" data-list="' + qq.CustomListId + '" onchange="checkChildList(this.id)" class="form-control" id="' + f.CustomFieldId + '"><option value="0">-- Select option --</option>';
                            }
                            else {
    ...
    $.ajax({
        ...
        success: function (cList) {
            ...
            if (!cList || !cList.CustomListDatas || cList.CustomListDatas.length < 1) {
                var childFildList = $('[data-pFId="' + controlId + '"]')[0];
                if (childFildList !== undefined && childFildList !== null && $.type(childFildList) === 'object') {
                    $('#frmGrp_' + $(childFildList).attr('id')).remove();
                }
...
function setChildListValue(listFieldId, crimsCustomData) {
    ...
    if (dataControl !== undefined && dataControl !== null) {
        ...
        if (crimsCustomData !== undefined && crimsCustomData !== null && crimsCustomData.length > 0) {
            ...
            if (data.length > 0) {
                dataControl.val(crimsCustomData).attr('data-edit-id', data[0].CustomDataId).select2();
            ...
            if (childData.length > 0) {
                dataControl.val(childData[0].CrimsCustomData).attr('data-edit-id', childData[0].CustomDataId).select2();
...
function saveThis(tabId) {
    ...
    if (selectList.length > 0) {
        for (var i = 0; i < selectList.length; i++) {
            ...
            var cData =
             {
                 CustomDataId: customDataControl.attr('data-edit-id'),
    ...
    if (textList.length > 0)
    {
        for (var j = 0; j < textList.length; j++) {
            ...
            var tetxtRequired = textDataControl.attr('data-required');
            var textData = textDataControl.val().trim();
            var type = textDataControl.attr('data-type');
            var size = textDataControl.attr('data-size');
            ...
            customDatas.push({
                CustomDataId: textDataControl.attr('data-edit-id'),
                CustomFieldId: textDataControl.attr('id'),
                EnrollmentId: $('#EnrollmentId').html(),
                CrimsCustomData: textData.toUpperCase(),
                CustomListId: null,
                ProjectSIteId: $('#ProjectPrimaryCode').html(),
    ...
    if (textAreaList.length > 0) {
        for (var k = 0; k < textAreaList.length; k++) {
            ...
            var textAreaRequired = textAreaDataControl.attr('data-required');
            ...
            customDatas.push({
                CustomDataId: textAreaDataControl.attr('data-edit-id'),
        ...
        if (dataControl !== undefined && dataControl !== null && dataControl.length > 0) {
            ...
            if (o.CustomListId === undefined || o.CustomListId === null || o.CustomListId.length < 1) {
                ...
                if (o.ParentListId === undefined || o.ParentListId === null || o.ParentListId.length < 1) {
                    dataControl.val(o.CrimsCustomData).attr('data-edit-id', o.CustomDataId).select2();
...
function getBioElements() {
    $.ajax({
        ...
        success: function (response) {
            ...
            if ((response.Photo === null || response.Photo.length < 1) && response.LeftLittle < 1 && response.LeftMiddle < 1 && response.LeftIndex < 1 && response.LeftThumb < 1 &&
                response.RightThumb < 1 && response.RightIndex < 1 && response.RightMiddle < 1 && response.RightRing < 1 && response.RightLittle < 1)
            {
                return;
            }
            ...
            if (response.FingerReasons !== null && response.FingerReasons.length > 0)
            {
                var reasonStr = '';
                response.FingerReasons.forEach(function (f)
                {
                    reasonStr += '<div class="col-md-12" style="margin-top: 5px"><label>' +
                        f.Name + '(' + f.FingerIndex + ')' + ' : <span style="font-weight : normal">' + f.Reason +' </span></label></div>';
                });

                $('#fingerReasonContainer').html(reasonStr);
                $('#fingerReasons').show();
            }
...
{
    var objs = JSON.parse(event.target.result);
    if (objs === null || objs === undefined || objs.length < 1)
    {
        alert('Backup file content could not be retrieved or the file is corrupted. Please try again later');
        return;
    }
    var processed = 0;
    var groupedArr = createGroupedArray(objs, 2);
        ...
        $.ajax({
            ...
            success: function (response)
            {
                if (response.Code < 1)
                {
                    ...
                    {
                        $('.showBusy').fadeOut();
                        alert(processed + " items were successsfully processed");
                        //location.reload();
                    }
                }
